...
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM *vm, void *reserved) {
    ...
    if (!cls) {
        /* Class only exists inside the engine app itself.
           In a protected target app it won't be present — that is normal.
           Clear the exception and skip RegisterNatives; the scripts bridge
           is never called at runtime in a protected app. */
