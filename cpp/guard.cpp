...
// ── §9.0  VMP late-init function pointer ─────────────────────────────────
// Assigned by jni_init.cpp's JNI_OnLoad (generated by GlobalDexConfig.java).
// Using a plain function pointer instead of a weak symbol because TCC does
// not reliably resolve __attribute__((weak)) function references from static
// archive objects at link time on Android ARM64/ARM.
// Guard §9 d3 reads this AFTER shards are merged — NativeUtil is available.
#ifdef __cplusplus
extern "C" {
#endif
...
Java_com_secure_dex_utils_DexProtector_install(JNIEnv *env, jclass /*cls*/, jobject context) {
    ...
    // g_vmp_lateInit_fp is set by jni_init.cpp's JNI_OnLoad (generated by
    // GlobalDexConfig.java) — using a plain function pointer instead of a
    // weak symbol because TCC does not reliably resolve weak symbols from
    // static archive objects at link time.
    // Must run AFTER §9 d merges the shards — NativeUtil lives inside them.
    if (g_vmp_lateInit_fp) { g_vmp_lateInit_fp(env); GLOGI("§9 d3: vmp_lateInit OK"); }
