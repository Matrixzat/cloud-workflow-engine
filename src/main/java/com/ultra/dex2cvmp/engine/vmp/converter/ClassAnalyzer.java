...
public class ClassAnalyzer {
    ...
    private boolean matchInterface(ClassDef classDef, String interfaceType) {
        ...
        for (String defInterface : classDef.getInterfaces()) {
            ...
            // Fix #3: was "return matchInterface(...)" which returned after the FIRST
            // interface regardless of the result, so only one interface was ever checked.
            // Now we continue the loop so all interfaces (and their hierarchies) are checked.
            final ClassDef ifClass = allClasses.get(defInterface);
            if (matchInterface(ifClass, interfaceType)) {
                return true;
            }
        }
        return false;
