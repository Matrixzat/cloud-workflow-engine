...
class Dex2C:
    ...
    def get_source_method(self, m):
        ...
        irmethod = z.process()
        if irmethod:
            # Return 3-tuple: (c_source, prototype, irmethod)
            # dex_bridge uses irmethod for optional VM compilation.
            # Existing callers only unpack [0] and [1] — backward compatible.
            return (irmethod.get_source(), irmethod.get_prototype(), irmethod)
        else:
