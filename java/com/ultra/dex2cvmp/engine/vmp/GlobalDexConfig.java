...
public class GlobalDexConfig {
    ...
    private void generateJniInitCode(Writer writer) throws IOException {
        ...
        writer.write(String.format(
                ...
                "}\n\n" +
                "// Called by guard.cpp §9 d3, AFTER VMP shards are merged into dexElements.\n" +
                "// NativeUtil lives inside those shards, so FindClass only works after §9 d.\n" +
                "// Declared weak-extern in guard.cpp so non-VMP builds link without errors.\n" +
                "JNIEXPORT void JNICALL vmp_lateInit(JNIEnv *env) {\n" +
                "%s" +
                "}\n",
