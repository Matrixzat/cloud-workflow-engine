...
public class FileUtils {
    ...
    private static boolean delAllFile(String path) {
        ...
        if (tempList == null) return flag; // I/O error listing directory — nothing to delete
        File temp;
    ...
    public static void deleteFloor(String dir) {
        ...
        if (files == null) return; // dir doesn't exist or I/O error — nothing to delete
        for (File file : files) {
            if (file.isFile()) {
    ...
    public static void deleteFiles(File path) {
        try {
            ...
            Object obj = contructor.newInstance(path.toString());
            Method method = main.getDeclaredMethod("delete");
            method.setAccessible(true);
            String[] entries = ((File) obj).list();
            if (entries == null) entries = new String[0]; // null when dir doesn't exist or I/O error
