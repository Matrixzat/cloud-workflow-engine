...
public class MyClassLoader {
    ...
    public void loadDexByVersion() throws Exception {
        ...
        // API 23+ (M+): use makePathElements (API 23-25) with fallback to
        // makeDexElements (renamed again on API 26 / Oreo+).
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            ...
            // Try makePathElements (API 23-25), fall back to makeDexElements (API 26+).
            Method factoryMethod = null;
            try {
                factoryMethod = Reflect.findMethod(pathList, "makePathElements", List.class, File.class, List.class);
            } catch (NoSuchMethodException ignored) {
                // API 26+ renamed it back to makeDexElements
            }
            if (factoryMethod == null) {
                factoryMethod = Reflect.findMethod(pathList, "makeDexElements", List.class, File.class, List.class);
            }

            ArrayList<IOException> suppressedExceptions = new ArrayList<>();
            Object[] addElements = (Object[]) factoryMethod.invoke(pathList, mDexFiles, mOptDir, suppressedExceptions);
